...
@ParentPackage("jshop")
@InterceptorRefs({ @InterceptorRef("goodsmoduleGoodsTypeTNInterecptor"),
...
public class GoodsTypeTNAction extends ActionSupport {
	...
	public String addGoodsTypeTNajax() {
		if (Validate.StrNotNull(this.getName())&&Validate.StrNotNull(this.getGoodsParameter())) {
			GoodsTypeTN gtn = new GoodsTypeTN();
			gtn.setGoodsTypeId(this.getSerial().Serialid(Serial.GOODSTYPE));
			gtn.setName(this.getName());
			gtn.setCreatetime(BaseTools.systemtime());
	...
	public String findAllGoodsTypeTN() {
		String tag = (String) ActionContext.getContext().getSession()
				.get(BaseTools.KEYFORAUTHORITY);
		if ("1".equals(tag)) {
			this.setSucflag(false);
			return "json";
		}
	...
	public void findDefaultAllGoodsTypeTN() {
		...
		if (Validate.StrNotNull(sortname) && Validate.StrNotNull(sortorder)) {
			String queryString = "from GoodsTypeTN  order by " + sortname + " "
					+ sortorder + "";
			List<GoodsTypeTN> list = this.getGoodsTypeTNService()
					.sortAllGoodsTypeTN(currentPage, lineSize, queryString);
	...
	public String findGoodsTypeTNById() {
		if (Validate.StrNotNull(this.getGoodsTypeId())) {
			List<GoodsTypeTN> list = this.getGoodsTypeTNService()
					.findGoodsTypeTNById(this.getGoodsTypeId().trim());
			if (!list.isEmpty()) {
				bean = list.get(0);
			}
		}
	...
	public String UpdateGoodsTypeTN() {
		if (Validate.StrNotNull(this.getName())
				&& Validate.StrNotNull(this.getGoodsParameter())) {
			GoodsTypeTN gtn = new GoodsTypeTN();
	...
	public String findGoodsTypeTNForSelect() {
		...
		List<GoodsTypeTN> list = this.getGoodsTypeTNService()
				.findAllGoodsTypeTNNopage();
		if (list != null) {
			this.setGoodstypetnlist("<option value='0'>---请选择---</option>");
			for (Iterator it = list.iterator(); it.hasNext();) {
				GoodsTypeTN gtn = (GoodsTypeTN) it.next();
				this.goodstypetnlist += "<option value='"
						+ gtn.getGoodsTypeId() + "'>" + gtn.getName()
						+ "</option>";
			}
	...
	public String findGoodsParameter() {
		if (Validate.StrNotNull(this.getGoodsTypeId())) {
			List<GoodsTypeTN> list = this.getGoodsTypeTNService()
					.findGoodsTypeTNById(this.getGoodsTypeId());
			if (!list.isEmpty()) {
				bean = list.get(0);
				this.setSucflag(true);
				return "json";
