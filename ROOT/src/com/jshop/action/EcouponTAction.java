...
public class EcouponTAction {
	...
	public String addEcouponT() {
		if (Validate.StrNotNull(this.getEcouponstate())
				&& Validate.StrNotNull(String.valueOf(this
						.getFavourableprices()))
				&& Validate
	...
	public String findAllEcouponT() {
		...
		int lineSize = rp;
		List<EcouponT> list = this.getEcouponTService().findAllEcoupon(
		...
		if (!list.isEmpty()) {
			...
			for (Iterator it = list.iterator(); it.hasNext();) {
				...
				cellMap.put(
						"cell",
						new Object[] { et.getGoodsname(), et.getGoodsid(),
								et.getEcouponstate(), et.getState(),
								BaseTools.formateDbDate(et.getBegintime()),
								BaseTools.formateDbDate(et.getEndtime()),
								et.getFavourableprices(),
								et.getPricededuction(), et.getNote() });
				rows.add(cellMap);
			}
			return "json";
		} else {
	...
	public String updateEcouponT() {
		if ((Validate.StrNotNull(this.getEcouponstate())
				...
				&& Validate
						.StrNotNull(String.valueOf(this.getPricededuction()))
				&& Validate.StrNotNull(this.getState()) && Validate
				.StrNotNull(this.getNote())) == true) {
			EcouponT et = new EcouponT();
