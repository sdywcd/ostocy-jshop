...
public class GoodsCategoryAction extends ActionSupport {
	...
	public void findGoodsCategoryT() {
		...
		List<GoodsCategoryT> list = this.getGoodsCategoryTService()
				.findAllGoodsCategoryBycreatorid(state);
		if (list != null) {
			int gradecount = 0;
	...
	public void findFooterCateogyrT() {
		...
		List<ArticleCategoryT> list = this.getArticleCategoryTService()
				.findArticleCategoryByposition(lineSize, status, position);
		if (list != null) {
	...
	public String searchGoodsByGoodsName() {
		...
		int lineSize = rp;
		List<GoodsT> list = this.getGoodsTService().findGoodsByGoodsname(
				currentPage, lineSize, this.getTopKeywords().trim());
		...
		if(!list.isEmpty()&&list!=null){
			...
			ActionContext.getContext().put("topKeywords", "");
			...
			PageModel<GoodsT> pm = new PageModel<GoodsT>(currentPage, lineSize,
					list, total);
			totalpage = total % lineSize == 0 ? totalpage : +1;
		...
		// 路径获取
		ActionContext.getContext().put(FreeMarkervariable.BASEPATH,
				this.getDataCollectionTAction().getBasePath());
		// 获取导航数据
		ActionContext.getContext().put(FreeMarkervariable.SITENAVIGATIONLIST,
				this.getDataCollectionTAction().findSiteNavigation());
		// 获取商城基本数据
		ActionContext.getContext().put(FreeMarkervariable.JSHOPBASICINFO,
				this.getDataCollectionTAction().findJshopbasicInfo());
		// 获取页脚分类数据
		ActionContext.getContext().put(FreeMarkervariable.FOOTCATEGORY,
				this.getDataCollectionTAction().findFooterCateogyrT());
		// 获取页脚文章数据
		ActionContext.getContext().put(FreeMarkervariable.FOOTERATRICLE,
